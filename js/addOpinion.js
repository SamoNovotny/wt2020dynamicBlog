...
function processMusic(music){
    ...
    if(music.classic || music.popMusic || music.jazz){
        ...
        if(music.popMusic){
            conclusion += ((conclusion==="Druhy hudby ktorÃ© poÄÃºvam: ")? "pop-music" : ", pop-music");
        }
        if(music.jazz){
            conclusion +=  ((conclusion==="Druhy hudby ktorÃ© poÄÃºvam: ")? "jazz" : ", jazz");
        }
    }
...
function processOpnFrmData(event){
    ...
    let nrvRepeatedRecension = (elements["repeatedRecension"].checked===true) ? elements["repeatedRecension"].value : "TÃºto strÃ¡nku recenzujem po prvÃ½krÃ¡t.";
    ...
    const newReview = {
        ...
        change: selectedOpt,
        name: nrvName,
        email: "mailto:" + nrvEmail + "?Subject=Reakcia%20na%20VaÅ¡u%20recenziu%20na%20blog%20s%20nÃ¡zvom%20Musicer",
...
function myReset(element) {
    ...
    let i = 0, len = subQuestionElems.length;
    for(; i<len; i++) {
