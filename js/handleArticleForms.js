...
function showFileUpload(){
    Array.from(document.getElementsByClassName("fsetFileUpload")).forEach(function (elm) {
        elm.style.display = "block"
    });
    Array.from(document.getElementsByClassName("btShowFileUpload")).forEach(function (elm) {
...
function uploadImg(serverUrl) {
    ...
    if (files.length>0){
        ...
        const fieldsetElement = Array.from(document.getElementsByClassName("fsetFileUpload"));
        const btShowFileUploadElement = Array.from(document.getElementsByClassName("btShowFileUpload"));
 ...
 */
function processArtEditFrmData(event, articleId, current, totalPages, serverUrl){
    ...
    fetch(`${serverUrl}/article/${articleId}`, postReqSettings)
        ...
        .finally(() => window.location.hash=`#article/${articleId}/${current}/${totalPages}`);
...
function processCommentForm(event, articleId, artCurrent, artTotalPages){
    ...
    fetch(url,postReqSettings)
        ...
        .catch(error => {
            window.alert(`BohuÅ¾iaÄ¾, komentÃ¡r sa nepodarilo pridaÅ¥ k ÄlÃ¡nku. ${error}`);

        })
        .finally(() => window.location.hash = `#article/${articleId}/${artCurrent}/${artTotalPages}`);
